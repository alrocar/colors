$(function(){
    ...
    for (elem in aaa) {
        $('#wrap').append(`
            <a href="#" class="hb">
                <!--div class="c">
                <img src="alrocar/fullsize/${aaa[elem]['name'].replaceAll('/', '').replaceAll(':', '').replaceAll('.', '')}" alt=""/>
                </div-->
            </a>
            <div class="fullBg ${aaa[elem]['url'].replaceAll('/', '').replaceAll(':', '').replaceAll('.', '')}">
                <img id="${aaa[elem]['url'].replaceAll('/', '').replaceAll(':', '').replaceAll('.', '')}" src=${aaa[elem]['url']} alt=""></img>
            </div>
        `)
    }
    ...
    issues.selectAll(".issue").each(function(issue, i) {
        d3.select(this).selectAll("color")
            .data(issue.colors.sort(function(x, y){
                return d3.descending(d3.hsl(x.hex).s, d3.hsl(y.hex).s);
                // return d3.descending(x.hex, y.hex);
            }))
            .enter()
            .append("div")
            .attr("class", "color")
    ...
    $("#vis").on("mouseenter", ".issue", function(event){
        ...
        var elem = $(`.${$(this).data("url").replaceAll('/', '').replaceAll(':', '').replaceAll('.', '')}.fullBg`);
        ...
        $(this).qtip({
            content: {
            button: true,
            text: "<img height='300' src=" + $(this).data("url") + " /> <strong>" + $(this).data("name") + "</strong>",
                title: ""
            },
            hide: {
                delay: 500,
